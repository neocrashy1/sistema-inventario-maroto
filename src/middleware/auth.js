...
export function authMiddleware(to, from, next) {
  ...
  if (to.meta.requiresAuth) {
    ...
    //   logger.warn(`Acesso negado: role '${to.meta.requiresRole}' requerida`)
    //   return next('/')
    // }
    
    // if (to.meta.requiresPermission && !authStore.checkPermission(to.meta.requiresPermission)) {
    //   logger.warn(`Acesso negado: permissão '${to.meta.requiresPermission}' requerida`)
    //   return next('/')
    // }
    
    // if (to.meta.requiresModule && !authStore.canAccessModule(to.meta.requiresModule)) {
    //   logger.warn(`Acesso negado: módulo '${to.meta.requiresModule}' não permitido`)
    //   return next('/')
    // }
  }
